...
public class TestExampleExt {
	...
	public void testR8(){
		...
		assertNotNull("Builder did not return any system.", hs);

		assertEquals("Wrong number of elements in system " + hs, 7, hs.size());

		for(Element e : hs.getElements()){
			if("MS".equals(e.getName())){
				assertEquals("Wring number of outputs in multi-split.",
							 3, e.getOutputs().length);
			}
		}
